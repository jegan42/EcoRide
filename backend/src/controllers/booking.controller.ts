...
export class BookingController {
  ...
  static readonly getById = async (req: Request, res: Response) => {
    ...
    const bookings = await BookingService.getById(user.id, id);
  ...
  static readonly getAllByTrip = async (req: Request, res: Response) => {
    ...
    const bookings = await BookingService.getAllByTripId(tripId);
    res.status(200).json({ bookings });
  };
  ...
  static readonly validate = async (
    ...
    try {
      ...
      const validateBookingMsg = await BookingService.validateBooking(
        id,
        user.id,
        action
      );
